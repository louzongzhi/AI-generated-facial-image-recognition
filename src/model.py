...
class C3k2(nn.Module):
    ...
    def forward(self, x):
        y1, y2 = torch.split(self.cv1(x), 2, dim=1)
        for m in self.m:
            y1 = m(y1)
        return self.cv2(torch.cat([y2, y1], dim=1))
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        N = H * W
        qkv = self.qkv(x)
        qkv = qkv.reshape([B, N + N, self.key_dim * 2 + self.head_dim]).permute(0, 2, 1)
        q, k, v = qkv.split([self.key_dim, self.key_dim, self.head_dim], dim=1)
        q = q * self.scale
        attn = torch.matmul(q, k.transpose(-2, -1))
        attn = F.softmax(attn, dim=-1)
        attn = torch.matmul(attn, v)
        attn = attn.permute(0, 2, 1).reshape([B, H, W, self.head_dim * self.num_heads])
